...
export const getAgentTool = new DynamicStructuredTool({
  ...
  func: async ({ agentAddress }) => {
    try {
      ...
      let metadata = null;
      const metadataURI = (agent as any).metadataURI;
      if (metadataURI) {
        try {
      ...
      const formattedAgent = {
        agentAddress: (agent as any).agentAddress,
        metadataURI: metadataURI,
        metadata: metadata,
        tokenId: (agent as any).tokenId.toString(),
        registeredAt: (agent as any).registeredAt.toString(),
