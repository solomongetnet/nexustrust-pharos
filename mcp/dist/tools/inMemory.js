...
import { saveItemInputSchema, getItemInputSchema, clearItemsInputSchema, } from "../validators/index.js";
import { inMemoryStore } from "../store/index.js";
export function registerInMemoryTools(server) {
    // Save an item
    server.registerTool("saveItem", {
    ...
    server.registerTool("getItems", {
        ...
        const itemsList = items
            .map((item) => `ID: ${item.id}, Content: "${item.content}", Created At: ${item.createdAt.toISOString()}`)
            .join("\n- ");
        return {
            content: [
                {
    ...
    // Clear all items
    server.registerTool("clearItems", {
